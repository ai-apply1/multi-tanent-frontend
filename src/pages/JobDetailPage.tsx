      ...
      <div className="grid gap-4">
        <SectionCard>
          ...
          <div className="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <MetaCell label="Accept line">
              {custom?.acceptThreshold == null ? (
                <span className="text-ink-muted">Engine default</span>
              ) : (
                <span className="mono">{custom.acceptThreshold}</span>
              )}
            </MetaCell>
            <MetaCell label="Reject line">
              {custom?.rejectThreshold == null ? (
                <span className="text-ink-muted">Engine default</span>
              ) : (
                <span className="mono">{custom.rejectThreshold}</span>
              )}
            </MetaCell>
            <MetaCell label="Vetting metrics">
              <span className="mono">
                {custom?.metrics.length ?? 0}
              </span>
            </MetaCell>
          </div>
        </SectionCard>
