...
import {
  ...
  fetchDemoVideoStatus,
  initDemoVideoUpload,
  retryDemoVideoTranscode,
  uploadDemoVideoToPresignedUrl
} from "@/features/demo-video/demoVideoApi"
import type { DemoVideoStatus } from "@/features/demo-video/types"
import axios from "axios"
import ApplyVideoPlayer from "@/components/apply-video/ApplyVideoPlayer"

// File-picker accept list — mirror the apply-video page so the admin's
// expectations carry across. The backend doesn't enforce a server-side
// allow-list; ffmpeg fails loudly on a non-video and surfaces in `lastError`.
 ...
 * and `"Uploading segments to storage (3/12)"`.
 */
...
export function DemoVideoPage() {
  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      try {
        const presigned = await initDemoVideoUpload({
        ...
        queryClient.invalidateQueries({ queryKey: ["demoVideoStatus"] })
      } catch (err) {
  ...
  const status: DemoVideoStatus = data?.videoStatus ?? "draft"
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            A short walkthrough shown on the AI interview intro page, before the
            candidate starts. Replace it any time, the new bundle goes live
            atomically when the transcode finishes.
          </p>
        </div>
        <Button
          variant="outline"
        ...
        <CardContent className="space-y-4 pt-6">
          {isError && (
            <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">
              Could not load demo-video status.{" "}
              <button
                onClick={() => refetch()}
        ...
        <CardContent>
          <div className="flex flex-col items-start gap-3 sm:flex-row sm:items-center">
            ...
            <p className="text-xs text-muted-foreground">
              The new bundle goes live the moment the transcode finishes.
              Candidates opening the interview see the new video on next load.
            </p>
          </div>
          <input
            ref={fileInputRef}
      ...
      {/* Preview card — reuses the same custom HLS player the apply-video
          page uses, pointed at the demo-video manifest. Key on `readyAt`
      ...
      {data?.hasReadyVideo && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              The exact stream candidates will see on the interview intro page.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ApplyVideoPlayer
              key={data.readyAt ?? "ready"}
              manifestUrl="/api/demo-video/manifest.m3u8"
